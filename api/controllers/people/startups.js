...
var actionGetAllItems = function (options) {
    ...
    if (req.query.q !== undefined & req.query.q !== null) {
        ...
        q = '%' + qraw.replace(/\s/gi,'%') + '%';
        querySQL = querySQL
            + 'SELECT DISTINCT startups.*'
            + ' FROM startups'
            + ' LEFT JOIN people_startups ON people_startups.startup_id = startups.id'
            + ' WHERE (startups.name LIKE ?)'
            + ' AND startups.id NOT IN ('
            +       'SELECT startup_id FROM people_startups WHERE person_id = ?'
            + ')'
            + ';'
...
var actionGetPersonItems = function (options) {
    ...
    var places = [];
    querySQL = querySQL
        + 'SELECT DISTINCT people_startups.person_id, people_startups.startup_id'
...
var getLabItemDetails = function (options) {
    ...
    var places = [];
    querySQL = querySQL
        + 'SELECT labs_startups.*,'
        + ' labs.name AS lab_name, labs.short_name AS lab_short_name'
        + ' FROM labs_startups'
        + ' JOIN labs ON labs.id = labs_startups.lab_id'
        + ' WHERE labs_startups.startup_id = ?'
        + ';';
    places.push(item.startup_id)
    return sql.getSQLOperationResult(req, res, querySQL, places,
        (resQuery, options) => {
...
var getPersonItemDetails = function (options) {
    ...
    querySQL = querySQL
        ...
        + ' FROM people_startups'
        + ' JOIN people ON people.id = people_startups.person_id'
        + ' WHERE people_startups.startup_id = ?'
        + ';';
    places.push(item.startup_id)
    return sql.getSQLOperationResult(req, res, querySQL, places,
        (resQuery, options) => {
...
var addPersonItem = function (options) {
    ...
    var places = [];
    querySQL = querySQL
        + 'INSERT INTO people_startups'
        + ' (person_id, startup_id)'
        +' VALUES (?,?);';
    places.push(
...
var deleteItemPerson = function (options) {
    ...
    var places = [];
    querySQL = querySQL
        + 'DELETE FROM people_startups WHERE person_id = ? AND startup_id = ?;'
...
var deleteItemLab = function (options) {
    ...
    var places = [];
    querySQL = querySQL
        + 'DELETE FROM labs_startups WHERE lab_id = ? AND startup_id = ?;'
