...
<v-card flat>
    ...
    <v-container>
        ...
        <v-row>
            <p class="mt-2"> To avoid duplications, please search below to find if dataset isn't
...
export default {
    ...
    methods: {
        initialize () {
            ...
            let urlSubmit = 'api/people/' + personID  + '/datasets';
        ...
        searchItems (search) {
            ...
            if (search.length > minimumLength) {
                ...
                let personID = this.otherPersonId;
                let urlSubmit = 'api/people/' + personID + '/all-datasets'
                                + '?';
