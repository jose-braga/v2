<template>
        <ul>
            <li v-for="(pos, j) in data.current_positions" :key="j">
                <span class="position-name">{{pos.administrative_position_name_en}} </span> -
                <span class="lab-name">{{pos.administrative_office_name_en}} @ </span>
    ...
    methods: {
        initialize () {
            ...
            if (this.$store.state.session.loggedIn) {
                ...
                subUtil.getInfoPopulate(this, 'api/people/' + personID + '/administrative-affiliations', true)
                .then( (result) => {
