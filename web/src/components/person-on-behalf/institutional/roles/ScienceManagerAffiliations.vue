<template>
        <ul>
            <li v-for="(pos, j) in data.current_positions" :key="j">
                <span class="position-name">{{pos.science_manager_position_name_en}} </span> -
                <span class="lab-name">{{pos.science_manager_office_name_en}} @ </span>
        ...
        initialize () {
            ...
            this.data.past_positions = [];
             if (this.$store.state.session.loggedIn) {
                ...
                subUtil.getInfoPopulate(this, 'api/people/' + personID + '/science-management-affiliations', true)
                .then( (result) => {
