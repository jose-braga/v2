...
<v-card flat>
    ...
    <v-container>
        <v-data-table
            ...
            :sort-by="['item_details.start_show']"
            :sort-desc="[true]"
            multi-sort
                            ...
                            <v-row>
                                ...
                                <v-col cols="6" sm="3">
                                    <v-menu ref="date_menu"
                                        v-model="data.newItem.show_start_date"
                                        ...
                                        <v-date-picker v-model="data.newItem.start"
                                            @input="data.newItem.show_start_date = false"
                                            no-title
                                        ></v-date-picker>
                                    </v-menu>
                                </v-col>
                                <v-col cols="6" sm="3">
                                    <v-menu ref="date_menu"
                                        v-model="data.newItem.show_end_date"
                                        :close-on-content-click="false"
                                        :nudge-right="10"
                                        transition="scale-transition"
                                        offset-y min-width="290px">
                                        ...
                                        <v-date-picker v-model="data.newItem.end"
                                            @input="data.newItem.show_end_date = false"
                                            no-title
                                        ></v-date-picker>
                                    </v-menu>
                                </v-col>
        ...
        <v-row>
            ...
            <v-col cols="12">
                <v-text-field
                    ...
                    label="Search startup name (char > 3)"
                    single-line
                    hide-details
                    class="px-2 mb-4"
                ></v-text-field>
        ...
        <v-form  ref="fromDB" @submit.prevent="submitNewAssociations">
            ...
            <v-row align-content="center" justify="end" class="mt-4">
                <v-col cols="2" align-self="end">
                    <v-row justify="end">
                        <v-btn type="submit"
                        outlined color="blue">Add to your startups</v-btn>
                    </v-row>
                </v-col>
                <v-col cols="1">
                    <v-progress-circular indeterminate
...
export default {
    ...
    data() {
        return {
            ...
            editedItem: {},
            headers: [
                { text: 'Startup name', value:'item_details.name' },
                { text: 'Started', value:'item_details.start_show' },
                { text: 'Ended', value:'item_details.end_show' },
    ...
    methods: {
        initialize () {
            ...
            let urlSubmit = 'api/people/' + personID  + '/startups';
            subUtil.getInfoPopulate(this, urlSubmit, true)
            .then( (result) => {
                for (let ind in result) {
                    if (result[ind].item_details.start) {
                        result[ind].item_details.start = time.momentToDate(result[ind].item_details.start);
                    }
                    result[ind].item_details.start_show = result[ind].item_details.start;
                    if (result[ind].item_details.end) {
                        result[ind].item_details.end = time.momentToDate(result[ind].item_details.end);
                    }
                    result[ind].item_details.end_show = result[ind].item_details.end;
                }
                this.data.items = result;
                if (this.dialog && this.editedIndex > -1) {
                    for (let ind in this.data.items) {
                        if (this.data.items[ind].startup_id === this.itemID) {
        ...
        searchItems (search) {
            ...
            if (search.length > minimumLength) {
                ...
                let personID = this.$store.state.session.personID;
                let urlSubmit = 'api/people/' + personID + '/all-startups'
                                + '?';
