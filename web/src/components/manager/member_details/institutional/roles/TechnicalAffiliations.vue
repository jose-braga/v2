            ...
            this.data.current_positions = [];
             if (this.$store.state.session.loggedIn) {
                ...
                subUtil.getInfoPopulate(this, 'api' + this.endpoint
                                + '/members'
                                + '/' + personID + '/technical-affiliations', true)
                .then( (result) => {
            ...
            if (this.$store.state.session.loggedIn) {
                    ...
                    if (affiliations[ind].id === 'new') {
                        ...
                        urlCreate.push({
                            url: 'api' + this.endpoint
                                + '/members'
                                + '/' + personID + '/technical-affiliations',
                        ...
                        urlUpdate.push({
                            url: 'api' + this.endpoint
                                + '/members'
                                + '/' + personID
                                + '/technical-affiliations/' + affiliations[ind].id,
                            body: affiliations[ind],
                        });
                    }
                }
                for (let ind in this.toDelete) {
                    urlDelete.push('api' + this.endpoint
                                    + '/members'
                                    + '/' + personID
                                    + '/technical-affiliations/' + this.toDelete[ind].id);
                }
                this.$http.all(
