...
<v-card>
    <v-form @submit.prevent="submitForm(publicationDetails)">
        ...
        <v-card-text>
            ...
            <v-container v-if="!loadingJournals">
                <v-row justify="end">
                    <span class="small">Last updated: {{ publicationDetails.updated }}</span>
                </v-row>
                <v-row>
                    <v-col cols="12" md="3">
                        <v-select multiple
...
export default {
    ...
    updated () {
        if (this.publicationDetails.isORCID) {
            this.$root.$emit('managerUpdateSingleAddPublicationDatabaseORCID',
                this.publicationDetails);
        } else if (this.publicationDetails.isPURE) {
            this.$root.$emit('managerUpdateSingleAddPublicationDatabasePURE',
                this.publicationDetails);
        } else {
            this.$root.$emit('managerUpdateSingleAddPublicationDatabase',
                this.publicationDetails);
        }
    },
    watch: {
